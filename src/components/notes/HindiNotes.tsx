...
import { FaBook, FaPenNib, FaAward, FaGraduationCap, FaLightbulb, FaCheckCircle } from "react-icons/fa";
...
const HindiNotes = () => {
    ...
    // Map ActiveTab string index to Chakra's number index
    const tabIndexMap: Record<TabType, number> = { explanations: 0, grammar_vocab: 1, questions: 2, textbook: 3 };
    const tabStringMap: TabType[] = ["explanations", "grammar_vocab", "questions", "textbook"];
    ...
    const filteredContent = useMemo(() => {
        ...
        switch (activeTab) {
            case "explanations":
                return notesList.filter(item => item.nodeType === "narrative_note" && !item.sectionOrTheme?.includes("शिल्प सौंदर्य"));
            case "grammar_vocab":
                return notesList.filter(item => item.sectionOrTheme?.includes("शिल्प सौंदर्य") || item.sectionOrTheme?.includes("भाषा") || (item.analysis?.contextualVocabulary && item.analysis.contextualVocabulary.length > 0));
            case "questions":
                return notesList.filter(item => (item.cbseExtractBasedQuestions && item.cbseExtractBasedQuestions.length > 0) || (item.cbseCompetencyLongAnswers && item.cbseCompetencyLongAnswers.length > 0) || (item.quickCheckMCQs && item.quickCheckMCQs.length > 0));
            case "textbook":
                return notesList.filter(item => item.nodeType === "textbook_exercise_section" || (item.fullTextbookSolutions && item.fullTextbookSolutions.length > 0));
            default:
                return notesList;
        }
    }, [notesList, activeTab]);
            ...
            <Container maxW="container.lg">
                ...
                {!contentLoading && notesList.length > 0 && (
                    <Stack spacing={6}>
                        ...
                        {/* Summary Panel */}
                        {chapterSummary && (
                            <Box bg={summaryBg} border="1px" borderColor={summaryBorder} p={{ base: 4, md: 6 }} borderRadius="2xl" boxShadow="xs">
                                <HStack spacing={2} mb={3}>
                                    <Icon as={FaBook} color="amber.600" boxSize={5} />
                                    <Heading as="h2" size="md" color="amber.900">पाठ का सारांश (Chapter Summary)</Heading>
                                </HStack>
                        ...
                        <Box bg={cardBg} borderRadius="2xl" boxShadow="sm" overflow="hidden" border="1px" borderColor={useColorModeValue("gray.100", "gray.700")} p={2}>
                            ...
                            >
                                <TabList
                                    ...
                                    p={1}
                                >
                                    <Tab borderRadius="xl" fontSize={{ base: "xs", md: "sm" }} py={{ base: 2, md: 3 }} px={3} flex={{ base: "1 1 45%", md: "1" }}><Icon as={FaBook} mr={1.5} display={{ base: "none", sm: "inline" }} />व्याख्या</Tab>
                                    <Tab borderRadius="xl" fontSize={{ base: "xs", md: "sm" }} py={{ base: 2, md: 3 }} px={3} flex={{ base: "1 1 45%", md: "1" }}><Icon as={FaPenNib} mr={1.5} display={{ base: "none", sm: "inline" }} />भाषा-शिल्प</Tab>
                                    <Tab borderRadius="xl" fontSize={{ base: "xs", md: "sm" }} py={{ base: 2, md: 3 }} px={3} flex={{ base: "1 1 45%", md: "1" }}><Icon as={FaAward} mr={1.5} display={{ base: "none", sm: "inline" }} />बोर्ड प्रश्न</Tab>
                                    <Tab borderRadius="xl" fontSize={{ base: "xs", md: "sm" }} py={{ base: 2, md: 3 }} px={3} flex={{ base: "1 1 45%", md: "1" }}><Icon as={FaGraduationCap} mr={1.5} display={{ base: "none", sm: "inline" }} />एनसीईआरटी</Tab>
                                </TabList>

                                <TabPanels mt={4}>
                                    {/* We handle map dynamically based on active tab state to preserve existing business logic structure */}
                                    <TabPanel p={0}>
                                        {/* Managed Below inside filteredContent map */}
                                    </TabPanel>
                                </TabPanels>
                            </Tabs>

                            {/* Core Content Nodes Loop */}
                            <Stack spacing={5} mt={2} p={{ base: 2, md: 4 }}>
